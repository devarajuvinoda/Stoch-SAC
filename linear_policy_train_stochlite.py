# ### Stochlite Training code using ARS
# Last Update by Tejas Rane (May, 2021)

"""
Policy Structure - 150 element (15*10) linear policy transforming 10 element state space to 15 element action space.

State space is defined as:
[Body_Roll, Body_Pitch, Body_Roll_Vel, Body_Pitch_Vel, Body_Yaw_Vel, Support_Plane_Roll, Support_Plane_Pitch, cmd_lin_x_vel, cmd_lin_y_vel, cmd_ang_z_vel]

Action space is defined as:
action[:4] = x_shift fl fr bl br
action[4:8] = y_shift fl fr bl br
action[8:12] = z_shift fl fr bl br
action[12] = augmented linear x velocity of robot (lin_x_vel)
action[13] = augmented linear y velocity of robot (lin_y_vel)
action[14] = augmented angular velocity of robot about z (ang_z_vel)
"""
...
class HyperParameters():
    ...
    def __init__(self, stairs=False, action_dim=15, normal=True, gait='trot', msg='', nb_steps=10000,
                 episode_length=400, learning_rate=0.02, nb_directions=16, nb_best_directions=8, noise=0.03, seed=1,
                 env_name='Stochlite-v0', curilearn=60, evalstep=3):
...
def ExploreWorker(rank, childPipe, envname, args):
    ...
    while True:
        ...
        if message == _EXPLORE:
            ...
            num_plays = 0
            sum_rewards = 0
            # js = j.Joystick(hp.episode_length, hp.curilearn)
            # commands = np.array(js.get_commands(hp.choice))
            # while num_plays < hp.episode_length:
            #     env.commands = commands[num_plays]
            #     state = np.concatenate((obs, commands[num_plays])).ravel()
...
class Policy():
    ...
    def __init__(self, input_size, output_size, env_name, normal, args, mask):
        try:
            ...
            if (normal):
                ...
                # self.theta[12][7] = 0.5 # cmd Vx to aug Vx
                # self.theta[13][8] = 0.5 # cmd Vy to aug Vy
                # self.theta[14][9] = 0.5 # cmd Wz to aug Wz
    ...
    def evaluate(self, input, delta, direction, hp):
        if direction is None:
            p = self.theta
            # for i in range(0,12): p[i][7:] = 0 # Policy Mask
            return np.clip(p.dot(input), -1.0, 1.0)
    ...
    def sample_deltas(self):
        return [np.random.randn(*self.theta.shape)*self.mask for _ in range(hp.nb_directions)] # Generating Masked delta matrix
...
def policyevaluation(env, policy, hp):
    ...
    if hp.domain_Rand:
        ...
        # --------------------------------------------------------------
        incline_deg_range = [0, 1, 2, 3, 4] # 5, 7, 9, 11, 13 deg
        incline_ori_range = [0]
        ...
        for j in incline_deg_range:
            for i in incline_ori_range:
                env.randomize_only_inclines(default=False, idx1=j, idx2=i)
                reward_evaluation = reward_evaluation + explore(env, policy, None, None, hp)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--env', help='Gym environment name', type=str, default='Stochlite-v0')
    ...
    parser.add_argument('--policy', help='Starting policy file (npy)', type=str, default='init_lite')
    ...
    parser.add_argument('--episode_length', help='length of each episode', type=int, default=100)
    ...
    parser.add_argument('--action_dim', help='degree of the spline polynomial used in the training', type=int,default=15)
    ...
    hp.nb_directions = 19 # = no of elements in the masked policy # int(env.observation_space.sample().shape[0] * env.action_space.sample().shape[0])
    ...
    policy_mask = np.load('utils/policy_mask_allcmd_19.npy') # Define the policy mask file here
    policy = Policy(nb_inputs, nb_outputs, hp.env_name, hp.normal, args, policy_mask)
