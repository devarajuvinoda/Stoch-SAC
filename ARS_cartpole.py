...
def main(step_size=0.5, nu=0.5, H=np.inf, num_individuals=50, num_selected=50, measure_step=100, num_episodes=5000,
    ...
    for iteration in range(num_iter):
        ...
        # print('rewards_plus:',rewards_plus)
        ...
        # print('best_relative_rewards:',best_relative_rewards)
        std = np.std(best_relative_rewards)
        # print('np.expand_dims(best_relative_rewards, axis=1):',np.expand_dims(best_relative_rewards, axis=1))
        # print('(np.expand_dims(np.expand_dims(best_relative_rewards, axis=1), axis=1)).sum(0):',(np.expand_dims(np.expand_dims(best_relative_rewards, axis=1), axis=1)).sum(0))
        # print('translations[best_idx]:',translations[best_idx])
    ...
    plot_learning_curve(x_plt, score_plt, 'ars_cartpole.png')
