...
import stoch_gym.envs.trajectory_generator as trajectory_generator
import math
import random
...
class StochliteEnv(gym.Env):
    ...
    def hard_reset(self):
        ...
        model_path = 'stoch_gym/envs/robots/stochlite/stochlite_description/urdf/stochlite_urdf.urdf'
    ...
    # 				round(cmd_translation_vel_reward, 4) + round(cmd_rotation_vel_reward, 4)\
    ...
    def _get_reward(self):
        ...
        '''
        wedge_angle = self.incline_deg*PI/180
        robot_height_from_support_plane = 0.25	
        pos, ori = self.GetBasePosAndOrientation()
        ...
        roll_reward = np.exp(-45 * ((RPY[0]-self.support_plane_estimated_roll) ** 2))
        pitch_reward = np.exp(-100 * ((RPY[1]-self.support_plane_estimated_pitch) ** 2))
    ...
    def GetObservation(self):
        ...
        obs = np.concatenate((obs, self.commands))
