...
if __name__ == '__main__':
	...
	parser.add_argument('--PolicyDir', help='directory of the policy to be tested', type=str, default='12Feb1')
	...
	# policy = np.load("experiments/"+args.PolicyDir+"/iterations/best_policy.npy") # Loading the best policy
	policy = np.load("experiments/"+args.PolicyDir+"/iterations/policy_78.npy") # Loading the selected policy
	print(policy)
	...
	if(args.WedgeIncline == 0 or args.Stairs):
		...
		args.Downhill = True
	env = e.StochliteEnv(render=True, end_steps = args.EpisodeLength, wedge=WedgePresent, stairs = args.Stairs, downhill= args.Downhill, seed_value=args.seed,
	...
	js = j.Joystick(args.EpisodeLength)
